...
func TestAssignNodesToCache(t *testing.T) {
	...
	for _, testCase := range testCases {
		engine := getTestJindoCacheEngineNode(fakeClient, testCase.name, testCase.namespace, testCase.withRunTime)
...
func TestSyncScheduleInfoToCacheNodes(t *testing.T) {
	...
	for _, testcase := range testcases {
		engine := getTestJindoCacheEngineNode(c, testcase.fields.name, testcase.fields.namespace, true)
