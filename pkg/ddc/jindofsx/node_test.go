...
func getTestJindoFSxEngineNode(client client.Client, name string, namespace string, withRunTime bool) *JindoFSxEngine {
...
func TestAssignNodesToCache(t *testing.T) {
	...
	for _, testCase := range testCases {
		engine := getTestJindoFSxEngineNode(fakeClient, testCase.name, testCase.namespace, testCase.withRunTime)
...
func TestSyncScheduleInfoToCacheNodes(t *testing.T) {
	...
	for _, testcase := range testcases {
		engine := getTestJindoFSxEngineNode(c, testcase.fields.name, testcase.fields.namespace, true)
